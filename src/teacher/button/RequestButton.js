...
const RequestButton = ({status, item}) => {
  const [isLoading, setIsLoading] = useState(false);

  const {updateMakeupClassHandlerCxt} = useContext(MakeupClassContext);
  function submit() {
    // console.log('updatedSelection : ', updatedSelection);
    console.log('updatedSelection : ', item);
    updateMakeupClassHandlerCxt(item.UniqueId, item.Status, item.id);
  }
